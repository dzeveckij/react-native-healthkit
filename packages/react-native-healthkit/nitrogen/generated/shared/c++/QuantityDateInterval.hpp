///
/// QuantityDateInterval.hpp
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © Marc Rousavy @ Margelo
///

#pragma once

#if __has_include(<NitroModules/JSIConverter.hpp>)
#include <NitroModules/JSIConverter.hpp>
#else
#error NitroModules cannot be found! Are you sure you installed NitroModules properly?
#endif
#if __has_include(<NitroModules/NitroDefines.hpp>)
#include <NitroModules/NitroDefines.hpp>
#else
#error NitroModules cannot be found! Are you sure you installed NitroModules properly?
#endif
#if __has_include(<NitroModules/JSIHelpers.hpp>)
#include <NitroModules/JSIHelpers.hpp>
#else
#error NitroModules cannot be found! Are you sure you installed NitroModules properly?
#endif
#if __has_include(<NitroModules/PropNameIDCache.hpp>)
#include <NitroModules/PropNameIDCache.hpp>
#else
#error NitroModules cannot be found! Are you sure you installed NitroModules properly?
#endif



#include <chrono>

namespace margelo::nitro::healthkit {

  /**
   * A struct which can be represented as a JavaScript object (QuantityDateInterval).
   */
  struct QuantityDateInterval final {
  public:
    std::chrono::system_clock::time_point from     SWIFT_PRIVATE;
    std::chrono::system_clock::time_point to     SWIFT_PRIVATE;

  public:
    QuantityDateInterval() = default;
    explicit QuantityDateInterval(std::chrono::system_clock::time_point from, std::chrono::system_clock::time_point to): from(from), to(to) {}

  public:
    friend bool operator==(const QuantityDateInterval& lhs, const QuantityDateInterval& rhs) = default;
  };

} // namespace margelo::nitro::healthkit

namespace margelo::nitro {

  // C++ QuantityDateInterval <> JS QuantityDateInterval (object)
  template <>
  struct JSIConverter<margelo::nitro::healthkit::QuantityDateInterval> final {
    static inline margelo::nitro::healthkit::QuantityDateInterval fromJSI(jsi::Runtime& runtime, const jsi::Value& arg) {
      jsi::Object obj = arg.asObject(runtime);
      return margelo::nitro::healthkit::QuantityDateInterval(
        JSIConverter<std::chrono::system_clock::time_point>::fromJSI(runtime, obj.getProperty(runtime, PropNameIDCache::get(runtime, "from"))),
        JSIConverter<std::chrono::system_clock::time_point>::fromJSI(runtime, obj.getProperty(runtime, PropNameIDCache::get(runtime, "to")))
      );
    }
    static inline jsi::Value toJSI(jsi::Runtime& runtime, const margelo::nitro::healthkit::QuantityDateInterval& arg) {
      jsi::Object obj(runtime);
      obj.setProperty(runtime, PropNameIDCache::get(runtime, "from"), JSIConverter<std::chrono::system_clock::time_point>::toJSI(runtime, arg.from));
      obj.setProperty(runtime, PropNameIDCache::get(runtime, "to"), JSIConverter<std::chrono::system_clock::time_point>::toJSI(runtime, arg.to));
      return obj;
    }
    static inline bool canConvert(jsi::Runtime& runtime, const jsi::Value& value) {
      if (!value.isObject()) {
        return false;
      }
      jsi::Object obj = value.getObject(runtime);
      if (!nitro::isPlainObject(runtime, obj)) {
        return false;
      }
      if (!JSIConverter<std::chrono::system_clock::time_point>::canConvert(runtime, obj.getProperty(runtime, PropNameIDCache::get(runtime, "from")))) return false;
      if (!JSIConverter<std::chrono::system_clock::time_point>::canConvert(runtime, obj.getProperty(runtime, PropNameIDCache::get(runtime, "to")))) return false;
      return true;
    }
  };

} // namespace margelo::nitro
